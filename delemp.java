...
public class delemp extends JDialog implements ActionListener
{
	static delemp d;
	Container cn;
	JButton b45,b65;
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		if(ae.getSource()==b65)
		{
			try
			{
				...
				String cs=tf1.getText();
				int flag=1;
				ResultSet rs=st.executeQuery("Select emp_id from emp");
				while(rs.next()==true)
				{
				...
				if(flag==0)
				{
					...
					if(r==0)
					{
						...
						st.executeUpdate("delete from emp where emp_id='"+s+"'");
						st.executeUpdate("delete from salary where emp_id='"+s+"'");
					}
					else
					{
						tf1.setText("");
